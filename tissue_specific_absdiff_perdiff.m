...
% % % tissue files (paths of age matched)
% % % abq_tissue=dir(fullfile(abq_dir,tissue,'efields','*.nii'));
% % % rst_tissue=dir(fullfile(rst_dir,tissue,'efields','*.nii'));
% % % smc_tissue=dir(fullfile(smc_dir,tissue,'efields','*.nii'));
% % % smf_tissue=dir(fullfile(smf_dir,tissue,'efields','*.nii'));
% % % spms_tissue=dir(fullfile(spms_dir,tissue,'efields','*.nii'));

% tissue files (paths of HC 18) for brain EF percdiff absdiff
...
% mask files (paths of age matched)
% % % abq_masks=dir(fullfile(abq_dir,tissue,'masks','*.nii'));
% % % rst_masks=dir(fullfile(rst_dir,tissue,'masks','*.nii'));
% % % smc_masks=dir(fullfile(smc_dir,tissue,'masks','*.nii'));
% % % smf_masks=dir(fullfile(smf_dir,tissue,'masks','*.nii'));
% % % spms_masks=dir(fullfile(spms_dir,tissue,'masks','*.nii'));

% mask files (paths of HC 18)
abq_masks=dir(fullfile(abq_dir,tissue,'*.nii'));
rst_masks=dir(fullfile(rst_dir,tissue,'*.nii'));
smc_masks=dir(fullfile(smc_dir,tissue,'*.nii'));
smf_masks=dir(fullfile(smf_dir,tissue,'*.nii'));
spms_masks=dir(fullfile(spms_dir,tissue,'*.nii'));
...
for t=1:subjects
    ...
    % read masks
    abq_msk=spm_vol(fullfile(abq_masks(1).folder,abq_masks(1).name));
    rst_msk=spm_vol(fullfile(rst_masks(t).folder,rst_masks(t).name));
    smc_msk=spm_vol(fullfile(smc_masks(t).folder,smc_masks(t).name));
    smf_msk=spm_vol(fullfile(smf_masks(t).folder,smf_masks(t).name));
    spms_msk=spm_vol(fullfile(spms_masks(t).folder,spms_masks(t).name));
    % read tissue images
    ...
    spms_img=spm_read_vols(spms_vol);
     % read mask images
    abq_mimg=spm_read_vols(abq_msk);
    rst_mimg=spm_read_vols(rst_msk);
    smc_mimg=spm_read_vols(smc_msk);
    smf_mimg=spm_read_vols(smf_msk);
    spms_mimg=spm_read_vols(spms_msk);
    
    msk_size=length(abq_mimg(:));
    
    % compare fields: absolute difference, for that concatenate the masks
    % and fields
    
    % abq-rst
    abq_rst_msk=[abq_mimg(:),rst_mimg(:)]; % abq rst masks
    ...
    for i=1:msk_size
        if abq_rst_msk(i,1)==1 && abq_rst_msk(i,2)==1
    ...
    % abq-smc
    abq_smc_msk=[abq_mimg(:),smc_mimg(:)]; % abq smc masks
   ...
   for i=1:msk_size
        if abq_smc_msk(i,1)==1 && abq_smc_msk(i,2)==1
    ...
    % abq-smf
    abq_smf_msk=[abq_mimg(:),smf_mimg(:)]; % abq smf masks
    ...
    for i=1:msk_size
        if abq_smf_msk(i,1)==1 && abq_smf_msk(i,2)==1
    ...
    % abq-spms
    abq_spms_msk=[abq_mimg(:),spms_mimg(:)]; % abq spms masks
    ...
    for i=1:msk_size
        if abq_spms_msk(i,1)==1 && abq_spms_msk(i,2)==1
    ...
    % rst-spm
    rst_spms_msk=[rst_mimg(:),spms_mimg(:)]; % rst spms masks
   ...
   for i=1:msk_size
        if rst_spms_msk(i,1)==1 && rst_spms_msk(i,2)==1
    ...
    % rst-smc
    rst_smc_msk=[rst_mimg(:),smc_mimg(:)]; % rst smc masks
    ...
    for i=1:msk_size
        if rst_smc_msk(i,1)==1 && rst_smc_msk(i,2)==1
            rst_smc_absdiff(i,1)=abs(rst_smc_ef(i,1)-rst_smc_ef(i,2));
            rst_smc_perdiff(i,1)=(abs(rst_smc_ef(i,1)-rst_smc_ef(i,2))/abs(max(rst_smc_ef(i,1),rst_smc_ef(i,2))))*100;
        end     
    end
    ...
    % rst-smf
    rst_smf_msk=[rst_mimg(:),smf_mimg(:)]; % rst smf masks
    ...
    for i=1:msk_size
        if rst_smf_msk(i,1)==1 && rst_smf_msk(i,2)==1
    ...
    % smc-smf
    smc_smf_msk=[smc_mimg(:),smf_mimg(:)]; % smc smf masks
    ...
    for i=1:msk_size
        if smc_smf_msk(i,1)==1 && smc_smf_msk(i,2)==1
    ...
    % smc-spms
    smc_spms_msk=[smc_mimg(:),spms_mimg(:)]; % smc spms masks
    ...
    for i=1:msk_size
        if smc_spms_msk(i,1)==1 && smc_spms_msk(i,2)==1
    ...
    % spms-smf
    spms_smf_msk=[spms_mimg(:),smf_mimg(:)]; % spms smf masks
    ...
    for i=1:msk_size
        if spms_smf_msk(i,1)==1 && spms_smf_msk(i,2)==1
...
T1_perdiff.Properties.VariableNames={'abq_rst','abq_smc','abq_smf','abq_spm','rst_smc','rst_smf','rst_spms','smc_smf','smc_spm','spm_smf'};
